...
class KMeans(object):
    ...
    def initialize_centroids(self, data):
        ...
        for i in range(1, self.k):
            ...
            for j in range(len(self.centroids)):
                distances[j] = self.get_euclidean_distance(self.centroids.iloc[j, self.start_var:self.end_var], sliced_data)
            ...
            distances["min"] = distances.min(axis=1)
            index = distances["min"].idxmax()
    ...
    def get_euclidean_distance(self, point1, point2):
        ...
        if isinstance(point1, pd.Series) and isinstance(point2, pd.Series):
            dist = np.sqrt(np.sum([(a - b) * (a - b) for a, b in zip(point1, point2)]))
        
        elif isinstance(point1, pd.DataFrame) and isinstance(point2, pd.Series):
            dist = []
            for i, row in point1.iterrows():
                item = np.sqrt(np.sum([(a - b) * (a - b) for a, b in zip(row, point2)]))
                dist.append(item)

            dist = pd.Series(dist)

        elif isinstance(point1, pd.Series) and isinstance(point2, pd.DataFrame):
            dist = []
            for i, row in point2.iterrows():
                item = np.sqrt(np.sum([(a - b) * (a - b) for a, b in zip(point1, row)]))
                dist.append(item)
    ...
    def group_observations(self, data):
        ...
        mins = []
        idxs = []
        for k, row in distances.iterrows():
            idxs.append(k)
            mins.append(row.idxmin())

        groups = pd.Series(mins, idxs)
    ...
    def adjust_centroids(self, data, groups):
        ...
        centroids = grouped_data.groupby(groups).mean()
